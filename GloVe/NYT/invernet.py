...
from Embedding.glove import getPretrainedModel, pretrain, indexPretrainedEmbedding
...
def setupGPU():
    config = tf.compat.v1.ConfigProto(device_count={'GPU': 2, 'CPU': 2})
...
if __name__ == '__main__':
    ...
    for n in n_arr:
        for b in b_arr:
            ...
            with open('./logs/training_log_GloVe_n{}_b{}.txt'.format(hyperparameters['num_inference_samples'], hyperparameters['bin_size']), 'w') as logFile:
                # with redirect_stdout(logFile):
                    setArtifactPath(n, b)

                    setupResultLog('./results/results_GloVe_n{}_b{}.csv'.format(hyperparameters['num_inference_samples'], hyperparameters['bin_size']))
                    ...
                    vocabIndices = np.random.randint(0, len(target_words), size=(1, round(0.01*len(target_words)))).squeeze()
                    target_words = [target_words[i] for i in vocabIndices]

                    # target_words = ['reuters']
