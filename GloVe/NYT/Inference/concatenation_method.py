...
from Embedding.glove import getPretrainedModel, getPretrainedEmb, getVocabSize, getModelIdxToOutputIdx
...
def buildInferenceModel():
    ...
    dense1 = Dense(1024, activation='relu', kernel_initializer='he_normal', bias_initializer='he_normal')(input_layer)
    drop1 = Dropout(0.3)(dense1)
    # dense2 = Dense(512, activation='relu', kernel_initializer='he_normal', bias_initializer='he_normal')(drop1)
    dense3 = Dense(128, activation='relu', kernel_initializer='he_normal', bias_initializer='he_normal')(drop1)
    drop2 = Dropout(0.3)(dense3)
    # dense4 = Dense(32, activation='relu', kernel_initializer='he_normal', bias_initializer='he_normal')(dense3)
    dense5 = Dense(8, activation='relu', kernel_initializer='he_normal', bias_initializer='he_normal')(drop2)

    # dense6 = Dense(2, name='output', activation='softmax')(dense1)
    dense6 = Dense(1, name='output', activation='sigmoid')(dense5)

    inferenceModel = Model(inputs=[src_input, down_input], outputs=dense6)
...
def trainingEvaluation(numInferenceSamples):
    ...
    yTrue = joblib.load(getPath('cooc_binary_{}'.format(sampleNo)))
...
def testingEvaluation(gloveModel, target_word, target_word_neighbors):
    global inferenceModel
    
    emb = joblib.load(getPath('emb_101'))
    yTrue = joblib.load(getPath('cooc_binary_101'))

    maxP, maxR, maxF1, tn, fp, fn, tp, maxAuc = 0, 0, 0, 0, 0, 0, 0, 0

    try:
        prediction = inferenceModel.predict([getPretrainedEmb(), emb], batch_size=getVocabSize())

        thresholds = [0.5, 0.4, 0.3, 0.25, 0.2, 0.15, 0.1, 0.01, 0.001]
        maxF1 = 0

        pretrainedKeysList = list(getPretrainedModel())
        downstreamKeysList = list(gloveModel)
        ...
        for _threshold in thresholds:
            ...
            presentIdx = []
            for item in pretrainedKeysList:
                if item in target_word_neighbors and item in downstreamKeysList:
                    presentIdx.append(getModelIdxToOutputIdx()[pretrainedKeysList.index(item)])
        ...
        print('In testing evaluation, an exception has occurred during prediction - {}'.format(ex))
